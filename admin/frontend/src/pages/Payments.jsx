  ...
  return (
    <div style={{
      ...
      <div>
        ...
        {sub && !loading && <div style={{ fontSize: 11, color: '#9CA3AF', marginTop: 3 }}>{sub}</div>}
      </div>
      <div style={{
...
const STATUS_CONFIG = {
  successful: { text: '✅ Successful', bg: '#D1FAE5', fg: '#065F46' },
  failed:     { text: '❌ Failed',     bg: '#FEE2E2', fg: '#991B1B' },
  pending:    { text: '⏳ Pending',    bg: '#FFFBEB', fg: '#D97706' },
};
  ...
  return (
    <div style={{ flex: 1, overflowY: 'auto', padding: '24px 28px', background: '#F9FAFB', fontFamily: "'DM Sans', system-ui, sans-serif" }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        <div>
          <h2 style={{ fontSize: 20, fontWeight: 700, color: '#111827', margin: 0 }}>Payment Transactions</h2>
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          <button
            ...
            style={{ background: '#fff', border: '1px solid #D1D5DB', borderRadius: 8, padding: '8px 16px', fontSize: 12, fontWeight: 600, cursor: 'pointer', color: '#374151', display: 'flex', alignItems: 'center', gap: 6 }}
          >
            📥 Export CSV
          </button>
          <button
            onClick={fetchPayments}
            style={{ background: '#1A56DB', border: 'none', borderRadius: 8, padding: '8px 16px', fontSize: 12, fontWeight: 600, cursor: 'pointer', color: '#fff', display: 'flex', alignItems: 'center', gap: 6 }}
          >
            🔄 Refresh
          </button>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 12, marginBottom: 24 }}>
        <StatCard label="Total Transactions" value={stats.total} icon="💳" bg="#EFF4FF" fg="#1A56DB" loading={loading} />
        <StatCard label="Successful" value={stats.successful} icon="✅" bg="#F0FDF4" fg="#059669" loading={loading} />
        <StatCard label="Pending" value={stats.pending} icon="⏳" bg="#FFFBEB" fg="#D97706" loading={loading} />
        <StatCard label="Failed" value={stats.failed} icon="❌" bg="#FEF2F2" fg="#DC2626" loading={loading} />
        <StatCard label="Revenue Collected" value={formatINR(stats.totalAmount)} icon="💰" bg="#F0FDF4" fg="#059669" sub="from successful payments" loading={loading} />
      </div>
      ...
      <div style={{ background: '#fff', border: '0.5px solid #E5E7EB', borderRadius: 12, overflow: 'hidden', boxShadow: '0 1px 3px rgba(0,0,0,0.02)' }}>
        ...
        {error && (
          <div style={{ background: '#FEE2E2', border: '1px solid #FCA5A5', color: '#991B1B', padding: '12px 16px', margin: '16px 24px 0', borderRadius: 8, fontSize: 13 }}>
            ...
            <button onClick={fetchPayments} style={{ marginLeft: 12, background: 'none', border: 'none', color: '#991B1B', cursor: 'pointer', textDecoration: 'underline', fontSize: 12 }}>Retry</button>
          </div>
        )}
            ...
            <tbody style={{ fontSize: 13, color: '#374151' }}>
              {loading ? (
                ...
                paginated.map(p => {
                  ...
                  return (
                    <tr key={p.id} className="pay-row" style={{ borderBottom: '1px solid #F3F4F6' }}>
                      <td style={{ padding: '14px 8px', fontWeight: 700, color: '#111827' }}>#{p.id}</td>
                      <td style={{ padding: '14px 8px' }}>
                        <span style={{ fontWeight: 500, color: '#111827' }}>{p.service_name || 'Home Service'}</span>
                        <div style={{ fontSize: 11, color: '#9CA3AF', marginTop: 2 }}>Booking #{p.booking_id}</div>
                      </td>
                      <td style={{ padding: '14px 8px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                          <div style={{ width: 28, height: 28, borderRadius: '50%', background: '#EFF4FF', color: '#1A56DB', fontSize: 11, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                            {(p.user_name || 'C')[0].toUpperCase()}
                          </div>
                          <span>{p.user_name || 'Customer'}</span>
                        </div>
                      </td>
                      <td style={{ padding: '14px 8px', color: '#6B7280', fontFamily: 'monospace', fontSize: 11 }}>
                        {p.razorpay_order_id ? (
                      ...
                      <td style={{ padding: '14px 8px', color: '#6B7280', fontSize: 12 }}>{formatDate(p.created_at)}</td>
                    </tr>
                  );
                })
              )}
